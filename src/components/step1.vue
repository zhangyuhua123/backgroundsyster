<template>
  <div>
    <div class='alert layer-anim layui-layer' v-if='isaddnewstudents'>
      <div class="for2">
        ...
        <div class="el-message-box__content">
          ...
          <p>
            <span>性别:</span>
            <input type="radio" style='height:13px' @click="RadioChanged(1)" name='ss'>男
            <input type="radio" style='height:13px' @click="RadioChanged(2)" name='ss'>女
          </p>
    ...
    <div class='employer'>
      ...
      <div class="banner">
        <el-form :inline="true" :model="formInline" class="demo-form-inline">
          ...
          <el-form-item label="手机号" style="margin-left:35px;">
            ...
            <div class="error6" v-show='pleasewhitetell'>请输入正确的手机号</div>
          </el-form-item>
          <el-form-item>
      ...
      <div class="block">
        <el-pagination layout="prev, pager, next" @current-change='handleCurrentChange' :page-size='10' :total="totalCount">
        </el-pagination>
      </div>
      ...
      <div class="for1 layer-anim layui-layer" v-if='isshowstudentmessage'>
        ...
        <div class="el-message-box__content">
          ...
          <p>
            <span>性别:</span>
            <input type="radio" value='男' name='ss' @click="RadioChanged(1)" v-model='sex' checked>男
            <input type="radio" value='女' name='ss' @click="RadioChanged(2)"  v-model='sex' checked>女</p>
        </div>
      ...
      <div class="for4 layer-anim layui-layer" v-show="iscontraction">
        ...
        <div class="el-message-box__content">
          <p>
            <span style="display:inline-block; width:300px">主合约:{{productname}}</span>
  ...
  methods: {
        ...
        for (var i = 0; i < result.length; i++) {
         // console.log("i:" + result[i]);
          if (result[i].ContractService_Start) {
            ...
            result[i].ContractService_Start = new Date(parseInt(result[i].ContractService_Start.toString().replace("/Date(", "").replace(")/", ""))).toLocaleString();
          }
          if (result[i].ContractService_Start) {

            result[i].ContractService_End = new Date(parseInt(result[i].ContractService_End.toString().replace("/Date(", "").replace(")/", ""))).toLocaleString();
          }
    ...
    handleEdit1(index, row) {
      ...
      this.$http.get(url, {
        ...
        this.updateContractItem = row;
        this.value1 = row.ContractService_Start.substr(0, row.ContractService_Start.indexOf(' '));
        this.value2 = row.ContractService_End.substr(0, row.ContractService_End.indexOf(' '));
